...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (!userMention || user == message.author.id) return message.reply(payEmbed);
    ...
    if (isNaN(Number(args[1])) || Number(args[1]) === Infinity) return message.reply(payEmbed);
    if (!foxCoins || message.content.includes('-') || foxCoins > userData.coins) return message.reply(":x: | Você não tem FoxCoins o suficiente para transferir!");
    ...
    message.reply(`💸 **|** Você deseja mesmo transferir ${args[1]} FoxCoins para ${userMention.user}? \nA Equipe da Foxy **Não se responsabiliza** pelas FoxCoins perdidas, então certifique-se de estar transferindo para uma pessoa de confiança! \nÉ proibido o comércio de conteúdo NSFW(+18) em troca de FoxCoins!`).then(sentMessage => {
      ...
      collector.on('collect', () => {
        message.reply(`Você transferiu ${foxCoins} FoxCoins para ${userMention.user}`);

...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (!user) {
      return message.reply('Mencione alguém para poder roubar');
    }
    ...
    if (user == message.author.id) return message.reply('Você não pode se roubar!');
    ...
    if (author !== null && timeout - (Date.now() - author) > 0) {
      ...
      if (targetuser <= random) { return message.reply(moneyEmbed2); }

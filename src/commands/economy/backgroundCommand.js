module.exports = {
  ...
  async run(client, message, args) {
    ...
    switch (args[0]) {
      case 'reset':
        if (background2 == null) return message.inlineReply('Você não tem nenhum background para redefinir!');
        if (background2 == 'default_background.png') return message.inlineReply('Você não tem nenhum background para redefinir!');

        const time = await db.fetch(`time_${user.id}`);
        if (time !== null && timeout - (Date.now() - time) > 0) {
          const times = ms(timeout - (Date.now() - time));
          message.inlineReply(`Você não pode redefinir seu background! Tente novamente em **${times.hours}h ${times.minutes}m ${times.seconds}s**`);
        } else {
          db.add(`coins_${user.id}`, 9000);
          message.inlineReply('Desculpe pela incoveniência, eu redefini seu background para o padrão! Você recebeu 9000 FoxCoins de compensação');
      ...
      case 'winxp':
        ...
        message.inlineReply(winxp).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 1000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Windows XP**, ele já foi definido`);

              db.subtract(`coins_${user.id}`, 3000);
              db.set(`background_${user.id}`, 'windows_xp.png');
            }).catch(collected => {
              message.inlineReply("Tempo esgotado")
            })
        });
      ...
      case 'red':
        ...
        message.inlineReply(reddead).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 5000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Red Dead Redemption 2**, ele já foi definido`);
      ...
      case 'gta':
        ...
        message.inlineReply(gtasa).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 9000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Grand Theft Auto: San Andreas**, ele já foi definido`);
      ...
      case 'fnaf':
        ...
        message.inlineReply(fnaf).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 9000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Five Night's at Freddy's**, ele já foi definido`);
              db.subtract(`coins_${user.id}`, 9000);
      ...
      case 'foxy':
        ...
        message.inlineReply(vlog).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 5000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Foxy Vlogger**, ele já foi definido`);
              db.subtract(`coins_${user.id}`, 5000);
              db.set(`background_${user.id}`, 'foxy_vlogs.png');
            }).catch(collected => {
              message.inlineReply("Tempo esgotado, você demorou para reagir!")
            })
        });
        break;
      ...
      case 'lori':
        ...
        message.inlineReply(lori).then((sentMessage) => {
          sentMessage.react('✅');
          const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
          sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time'] })
            .then((collected) => {
              if (money < 10000) return message.inlineReply('Você não tem coins o suficiente para este background');
              message.inlineReply(`Você comprou o background **Foxy e Loritta**, ele já foi definido`);
        ...
        break;
          ...
          case 'sad_cat_money':
            ...
            message.inlineReply(money2).then((sentMessage) => {
              sentMessage.react('✅');
              const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
              sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time']})
              .then((collected) => {
                if(money < 10000) return message.inlineReply('Você não tem coins o suficiente para este background');
                message.inlineReply('Você comprou o background **Sad Cat Money**, ele já foi definido');
                db.subtract(`coins_${user.id}`, 10000);
            ...
            case 'levi':
              ...
              message.inlineReply(money3).then((sentMessage) => {
                sentMessage.react('✅');
                const filter = (reaction, user) => ['✅'].includes(reaction.emoji.name) && user.id === message.author.id;
                sentMessage.awaitReactions(filter, { max: 1, time: 120000, errors: ['time']})
                .then((collected) => {
                  if(money < 10000) return message.inlineReply('Você não tem coins o suficiente para este background')
                  message.inlineReply('Você comprou o background **Ata Shingeki No Kyojin**, ele já foi definido')
                  db.subtract(`coins_${user.id}`, 10000)

    ...
    async execute(ctx, t): Promise<void> {
        ...
        switch (command) {
            case "info": {
                ...
                const avatarCollector = ctx.channel.createMessageComponentCollector(filter, { max: 1, time: 5000 });
                    ...
                    if (i.customId === 'avatar') {
                        if (await ctx.getContext(i, 1)) {
                            if (i.user.id !== ctx.user.id) {
                                i.deferUpdate();
                                return i.user.send({ content: t('commands:foxyGlobal.noPermission', { user: `<@${ctx.user.id}>` }) });
                            }
                            ...
                            await ctx.followUp({ embeds: [avatarEmbed], ephemeral: true, components: [row] });
                            i.deferUpdate();
                        ...
                        if (await ctx.getContext(i, 1)) {
                            ...
                            ctx.followUp({ embeds: [embed], ephemeral: true });
                            i.deferUpdate();
                            avatarCollector.stop();
                        }
                    }
            ...
            case "avatar": {
                ...
                await ctx.reply({ embeds: [avatarEmbed], components: [row] });
                break;
            }

            case "banner": {
                if (!data.banner) return ctx.reply(t('commands:user.banner.noBanner'));
                ...
                if (!banner) return ctx.reply(t('commands:user.banner.noBanner'));
                const bannerEmbed = new EmbedBuilder()
                    .setTitle(t('commands:user.banner.title', { user: user.username }))
                    .setImage(banner)

                await ctx.reply({ embeds: [bannerEmbed], components: [row] });
            }
        }
    }
}
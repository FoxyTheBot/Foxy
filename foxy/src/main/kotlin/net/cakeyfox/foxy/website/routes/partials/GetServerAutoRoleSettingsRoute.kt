...
import frontend.htmx.partials.getAutoRoleSettings
...
class GetServerAutoRoleSettingsRoute {
    @OptIn(ExperimentalKtorApi::class)
    fun Routing.getAutoRoleSettings(server: FoxyWebsite) {
        route("/{lang}/partials/{guildId}/autorole") {
            hx.get {
                ...
                try {
                    ...
                    val alreadyAddedRoles = guild.AutoRoleModule?.roles ?: emptyList()
                    val roles = RouteUtils.getRolesFromAGuild(server, guildId, server.httpClient)

...
class MarryLetterExecutor : UnleashedCommandExecutor() {
    override suspend fun execute(context: CommandContext) {
        ...
        if (context.getAuthorData().userCakes.balance < storeItem.price) {
            context.reply {
                content = pretty(FoxyEmotes.FoxyCry, context.locale["marry.letter.youHaveNoCakes"])
            }
            return
        }
        ...
        context.reply(true) {
            embed {
                ...
                title = pretty(FoxyEmotes.Letter, context.locale["marry.letter.sendLoveLetter.title"])
                description = """
                    > $letterText
            ...
            actionRow(
                context.foxy.interactionManager.createButtonForUser(
                    ...
                    it.deferEdit()
                    val marriedWith = if (marriageInfo.firstUserId == context.user.id) {
                        context.foxy.shardManager.retrieveUserById(marriageInfo.secondUserId).await()
                    } else {
                        context.foxy.shardManager.retrieveUserById(marriageInfo.firstUserId).await()
                    }

                    context.database.user.updateMarriage(context.user.id) {
                        if (marriageInfo.firstUserId == context.user.id) {
                            incFirstUserLetters()
                        } else {
                    ...
                    if (storeItem.price != 0) {
                        context.database.user.removeCakesFromUser(context.user.id, storeItem.price.toLong())
                    }
